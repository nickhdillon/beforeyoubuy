<div>
    <section class="mt-12 border-t-2 border-dashed border-emerald-200 pt-8" aria-labelledby="wishlist-heading">
        <div class="flex flex-col">
            ...
            <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end">
                ...
                <flux:modal.trigger name="wishlist-item-filters">
            ...
            <flux:modal name="wishlist-item-filters" flyout variant="floating" class="md:w-96">
                <div class="grid gap-6">
                    <div>
                        <flux:heading size="lg" class="font-black!">Filter wishlist items</flux:heading>
                        <flux:text class="mt-2! font-medium! text-zinc-600!">Narrow your wishlist by its item details.</flux:text>
                    </div>

                    <flux:select variant="listbox" wire:model="filterDraft.tagId" label="Tag" placeholder="All tags" clearable>
                        @foreach ($this->tags as $tag)
                            <flux:select.option wire:key="wishlist-filter-tag-{{ $tag->id }}" value="{{ $tag->id }}">{{ $tag->name }}</flux:select.option>
                        @endforeach
                    </flux:select>
                    ...
                    <flux:select variant="listbox" wire:model="filterDraft.photo" label="Photo" placeholder="With or without" clearable>
                        <flux:select.option value="with">Has a photo</flux:select.option>
                        <flux:select.option value="without">No photo</flux:select.option>
                    </flux:select>
                    ...
                    <div class="flex items-center justify-between gap-3 border-t-2 border-dashed border-orange-200 pt-5">
                ...
                <div class="grid gap-5 border-2 border-dashed border-orange-300 bg-orange-50 p-6 sm:grid-cols-[auto_1fr_auto] sm:items-center">
                    ...
                    <div>
                        <h3 class="text-xl font-black tracking-tight">
                            {{ $this->hasActiveFilters() ? 'No matching wishlist items' : 'Nothing on your wishlist' }}
                        </h3>

                        <p class="mt-2 text-sm leading-relaxed font-medium text-zinc-600">
                            {{ $this->hasActiveFilters() ? 'Try changing or clearing one of your filters.' : 'Save an idea here before it turns into an impulse purchase.' }}
                        </p>
                    </div>
    ...
    <livewire:wishlist-items.form :$collection :$wishlist />
</div>
